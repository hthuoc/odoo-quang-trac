...
import json
import os
import ssl  # Thêm thư viện quản lý chứng chỉ bảo mật (Add security certificate management library)
...
# ===================================================================
# 2. CHỨC NĂNG 1: TRA CỨU MÃ QUANG TRẮC (BỘ CODE 1)
# ===================================================================
def fetch_lookup_data_from_odoo(query_str):
    ...
    for attempt in range(max_retries):
        try:
            ctx = ssl._create_unverified_context()  # Tạo ngữ cảnh bỏ qua kiểm tra chứng chỉ (Create context to bypass certificate verification)
            ...
            if not code_field_list:
                st.error("Chưa cấu hình đúng tên kỹ thuật cho 'Mã quang trắc' trong FIELDS_MAP.")
                return pd.DataFrame()
...
# ===================================================================
# 3. CHỨC NĂNG 2: QUẢN LÝ THỜI HẠN MÃ QUANG TRẮC (BỘ CODE 2)
# ===================================================================
...
def run_management_app():
    ...
    # -------------------------------------------------------------------
    # KIỂM TRA VÀ HIỂN THỊ KHUNG XÁC NHẬN CẢNH BÁO ĐẾN HẠN Ở PHÍA TRÊN
    # -------------------------------------------------------------------
    res_data_check = load_reschedule_data()
    now_dt_check = datetime.now()
    due_codes_list = []
    due_jobs_list = []

    for c_k, c_inf in res_data_check.get("codes", {}).items():
        if not c_inf.get("ack", False):
            res_dt = parse_reschedule_dt(c_inf["rescheduled_deadline"])
            if res_dt and now_dt_check >= res_dt:
                due_codes_list.append(c_k)

    for j_k, j_inf in res_data_check.get("jobs", {}).items():
